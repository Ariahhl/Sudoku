...
int main() {
  int it = 0, salam = 0, khobi = 0, abas = 1;
  int sudo[9][9], a1[9][9], a2[9][9], a3[9][9], a4[9][9], a5[9][9], a6[9][9], a7[9][9], a8[9][9], a9[9][9], tester[9][9], nontemp[9][9];
  for (int i = 0; i < 9; i++)
    for (int j = 0; j < 9; j++) {
      a1[i][j] = 1;
      a2[i][j] = 2;
  ...
  while (true) {
    ...
    if (dupcheck(tester, sudo) == 1 && it % 2 != 0) {
      for (int i = 0; i < 9; i++)
        for (int j = 0; j < 9; j++)
          sudo[i][j] = nontemp[i][j];
      if (sudo[salam][khobi] == 0) {
        sudo[salam][khobi] = abas;
      }
      abas++;
      if (abas == 9) {
        khobi++;
